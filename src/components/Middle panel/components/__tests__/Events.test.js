...
//testing  the share user story
it('should increase share by one when clicked',()=>{
    const { getByTestId } = render(<event.EventComponent event={ Eventdata[0] } />)
    expect(screen.getByTestId('sharebutton')).toBeInTheDocument();
    userEvent.click(screen.getByTestId('sharebutton'));
    expect(screen.getByTestId('numShares')).toHaveTextContent('24');

})
it('should not increase share when clicked somewhere else',()=>{
    const { getByTestId } = render(<event.EventComponent event={ Eventdata[0] } />)
    expect(screen.getByTestId('numShares')).toBeInTheDocument();
    userEvent.click(screen.getByTestId('numShares'));
    expect(screen.getByTestId('numShares')).toHaveTextContent('23');
